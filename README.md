...
This setup requires the local X server to be reachable over TCP as display
localhost:0 (TCP port 6000).
Test:
...
The client must provide an X11 server reachable at localhost:6000.
 ...
 - XWayland: use a localhost-only TCP→X11 Unix-socket bridg (e.g. something
   like "socat TCP-LISTEN:6000,bind=127.0.0.1,reuseaddr,fork \
